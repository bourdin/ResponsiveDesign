...
### Responsive Material
###
AddPlot("Pseudocolor", "Stimulus x", 1, 0)
AddOperator("Isovolume", 0)
SetActivePlots(1)
IsovolumeAtts = IsovolumeAttributes()
IsovolumeAtts.lbound = 0.5
IsovolumeAtts.ubound = 1e+37
IsovolumeAtts.variable = "Responsive"
...
###
### Apply Displacements and save
###
AddOperator("Displace", 1)
for lc in ["x","y"]:
    SetActivePlots(1)
    ChangeActivePlotsVar("Stimulus {}".format(lc))
    DisplaceAtts = DisplaceAttributes()
    DisplaceAtts.factor = 0.
    DisplaceAtts.variable = "{}-displacement".format(lc)
    SetOperatorOptions(DisplaceAtts, 2, 1)
    DrawPlots()

    # SaveWindowAtts = SaveWindowAttributes()
    SaveWindowAtts.fileName = "{}-design-{}".format(prefix,lc)
    SetSaveWindowAttributes(SaveWindowAtts)
    SaveWindow()

    DisplaceAtts = DisplaceAttributes()
    DisplaceAtts.factor = 0.05
    DisplaceAtts.variable = "{}-displacement".format(lc)
    SetOperatorOptions(DisplaceAtts, 2, 1)
    DrawPlots()

    # SaveWindowAtts = SaveWindowAttributes()
    SaveWindowAtts.fileName = "{}-disp-{}".format(prefix,lc)
    SetSaveWindowAttributes(SaveWindowAtts)
    SaveWindow()
DeleteAllPlots()
CloseDatabase(db)
